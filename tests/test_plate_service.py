...
class PlateServiceTests(unittest.TestCase):
    ...
    def test_reduced_plate_height_is_passed_to_blender(self):
        ...
        self.assertEqual(cmd[-4:-2], ["CENTER", "128"])
    ...
    def test_footer_text_is_passed_to_blender(self):
        ...
        cmd = run.call_args.args[0]
        self.assertEqual(cmd[-2], "Residencial Teste")

    def test_cut_side_is_passed_to_blender(self):
        result = plate_service.subprocess.CompletedProcess(
            args=["blender"], returncode=1, stdout="stdout log", stderr="stderr log"
        )

        with mock.patch("subprocess.run", return_value=result) as run:
            plate_service.generate_plate("Teste", 20, "CENTER", 128, "Condomínio Astro", "LEFT")

        cmd = run.call_args.args[0]
        self.assertEqual(cmd[-1], "LEFT")
